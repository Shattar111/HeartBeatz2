...
- (void)requestAuthorization {
    ...
    [self.healthStore requestAuthorizationToShareTypes:nil readTypes:[NSSet setWithObjects:type, type2, type3,type4, nil] completion:^(BOOL success, NSError * _Nullable error) {
    ...
    [self.healthStore requestAuthorizationToShareTypes:[NSSet setWithObjects:type,type2,type3,type4, nil] readTypes:[NSSet setWithObjects:type,type2,type3,type4, nil] completion:^(BOOL success, NSError * _Nullable error) {
        if (success) {
            NSLog(@"OK");
        }else{
            NSLog(@"error %@", error);
