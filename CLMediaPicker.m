...
- (void)viewDidLoad {
    ...
    [[UINavigationBar appearance] setBarTintColor:[UIColor colorWithRed:58.0/255.0 green:58.0/255.0 blue:58/255.0 alpha:1.0]];
...
- (void)doneButtonAction:(id)sender {
    ...
    int musiccounter = counter;
    NSString *whatever = [NSString stringWithFormat:@"Add %d songs to BPM playlist?", musiccounter];
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"HeartBeatz Music" message:whatever preferredStyle:UIAlertControllerStyleAlert];
...
-(void)AlertYesButton
{
    ...
    if ([self.delegate respondsToSelector:@selector(clMediaPicker:didPickMediaItems:)]) {
        ...
        for (MPMediaEntity *entity in self.pickedItems) {
            if ([entity isKindOfClass:[MPMediaItemCollection class]]) {
                ...
                //  NSLog (@"Your Array elements are = %@", collection);
                
                for (NSString *yourVar in items) {
    ...
    if ([self.delegate respondsToSelector:@selector(clMediaPickerDidDonePicking:)]) {
        [self.delegate performSelector:@selector(clMediaPickerDidDonePicking:) withObject:self];
    }
...
- (void)addButtonAction:(UIButton *)sender {
    ...
    {
        paintView=[[UIView alloc]initWithFrame:CGRectMake(140, 620, 150, 50)];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(22, 8, 120, 30)];
        //label.textAlignment = UITextAlignmentCenter;
        [label setFont:[UIFont boldSystemFontOfSize:17.0]];
        // [label setBackgroundColor:[UIColor clearColor]];
        [label setTextColor:[UIColor whiteColor]];
        [label setText:@"Music Added"];
        [paintView addSubview:label];
        [self.navigationController.view addSubview:paintView];
        
        counter++;
        [UIView animateWithDuration:1.5
                         animations:^{paintView.alpha = 0.0;}
                         completion:^(BOOL finished){ [paintView removeFromSuperview]; }];
    ...
    else if ([platformString isEqualToString:@"iPhone X"])
    {
        ...
        paintView=[[UIView alloc]initWithFrame:CGRectMake(120, 660, 150, 50)];
        [paintView setBackgroundColor:[UIColor blackColor]];
        //[self.view addSubview:paintView];
        paintView.alpha = 0.8;
        paintView.layer.cornerRadius = 5;
    ...
    {
        paintView=[[UIView alloc]initWithFrame:CGRectMake(110, 560, 150, 50)];
        [paintView setBackgroundColor:[UIColor blackColor]];
        //[self.view addSubview:paintView];
        paintView.alpha = 0.8;
        paintView.layer.cornerRadius = 5;
